...
public class RobotManager {
	...
	public static void setRobotPosition(int position) {
		...
		isRobotSet = true;
		MainControl.mainWindow.setRobotPosition(MapManager.idToX(robotPosition)
				+ "," + MapManager.idToY(robotPosition));
	}
		...
		if (isRobotSet) {
			...
			timer = new Timer(TIMER_DELAY, new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					MainControl.mainWindow.setTimerDisplay(timeElapsed / 1000
							+ "s" + timeElapsed % 1000 + "ms");
				}
			});
	...
	private static void move() {
		...
		if (robotPosX > START_X) {
			if (MapManager.exploredSpaces.contains(MapManager.XYToId(
					robotPosX - 1, robotPosY)) == false
					|| MapManager.exploredSpaces.contains(MapManager.XYToId(
							robotPosX - 1, robotPosY + 1)) == false
					|| ((robotPosX > START_X + 1) && (MapManager.exploredSpaces
							.contains(MapManager.XYToId(robotPosX - 2,
									robotPosY)) == false || MapManager.exploredSpaces
							.contains(MapManager.XYToId(robotPosX - 2,
									robotPosY + 1)) == false))
					|| ((robotPosY < END_Y) && (MapManager.exploredSpaces
							.contains(MapManager.XYToId(robotPosX - 1,
									robotPosY + 2)) == false))) {
				if (moveLeft()) {
					pause();
		...
		if (robotPosX < END_X - 1) {
			if (MapManager.exploredSpaces.contains(MapManager.XYToId(
					robotPosX + 2, robotPosY)) == false
					|| MapManager.exploredSpaces.contains(MapManager.XYToId(
							robotPosX + 2, robotPosY + 1)) == false
					|| ((robotPosX < END_X - 2) && (MapManager.exploredSpaces
							.contains(MapManager.XYToId(robotPosX + 3,
									robotPosY)) == false || MapManager.exploredSpaces
							.contains(MapManager.XYToId(robotPosX + 3,
									robotPosY + 1)) == false))
					|| ((robotPosY < END_Y) && (MapManager.exploredSpaces
		...
		if (robotPosY > START_Y) {
			if (MapManager.exploredSpaces.contains(MapManager.XYToId(robotPosX,
					...
					|| MapManager.exploredSpaces.contains(MapManager.XYToId(
							robotPosX + 1, robotPosY - 1)) == false
					|| ((robotPosY > START_Y + 1) && (MapManager.exploredSpaces
							.contains(MapManager.XYToId(robotPosX,
									robotPosY - 2)) == false || MapManager.exploredSpaces
							.contains(MapManager.XYToId(robotPosX + 1,
									robotPosY - 2)) == false))
		...
		if (robotPosY < END_Y - 1) {
			if (MapManager.exploredSpaces.contains(MapManager.XYToId(robotPosX,
					robotPosY + 2)) == false
					|| MapManager.exploredSpaces.contains(MapManager.XYToId(
							robotPosX + 1, robotPosY + 2)) == false
					|| ((robotPosY < END_Y - 2) && (MapManager.exploredSpaces
							.contains(MapManager.XYToId(robotPosX,
									robotPosY + 3)) == false || MapManager.exploredSpaces
							.contains(MapManager.XYToId(robotPosX + 1,
									robotPosY + 3)) == false))) {
				if (moveDown()) {
	...
	private static void senseUp(final int RANGE) {
		Hashtable<Integer, Integer> results = MapManager.robotSensing(
				MapManager.idToX(robotPosition),
	...
	private static void senseDown(final int RANGE) {
		Hashtable<Integer, Integer> results = MapManager.robotSensing(
				MapManager.idToX(robotPosition),
				MapManager.idToY(robotPosition) + ROBOT_HEIGHT, ROBOT_WIDTH,
				RANGE);
		moveStrategy.getMapUpdate(results);
	}
	...
	private static void senseLeft(final int RANGE) {
		Hashtable<Integer, Integer> results = MapManager.robotSensing(
				MapManager.idToX(robotPosition) - RANGE,
	...
	private static void senseRight(final int RANGE) {
		Hashtable<Integer, Integer> results = MapManager.robotSensing(
				MapManager.idToX(robotPosition) + ROBOT_WIDTH,
				MapManager.idToY(robotPosition), RANGE, ROBOT_HEIGHT);
		moveStrategy.getMapUpdate(results);
	}
	...
	protected static void getExplorationPercentage(double percentage) {
		MainControl.mainWindow.setMapExplored(String.format(
				"Map Explored: %.2f%%", percentage * 100.0));
	}
